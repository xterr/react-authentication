...
export default class OAuth2Provider extends AbstractProvider {
  private static CACHE_KEY_ACCESS_TOKEN = 'oauth_access_token';
  private static CACHE_KEY_REFRESH_TOKEN = 'oauth_refresh_token';
  ...
  public async initialize (): Promise<void> {
    const sAccessToken = await this._getRequiredOption<CacheInterface>('cache').get<string>(OAuth2Provider.CACHE_KEY_ACCESS_TOKEN);
    const sRefreshToken = await this._getRequiredOption<CacheInterface>('cache').get<string>(OAuth2Provider.CACHE_KEY_REFRESH_TOKEN);
  ...
  public async login (options: LoginOptions): Promise<void> {
    ...
    await this._getRequiredOption<CacheInterface>('cache').set(OAuth2Provider.CACHE_KEY_ACCESS_TOKEN, options.accessToken);
    ...
    await this._getRequiredOption<CacheInterface>('cache').set(OAuth2Provider.CACHE_KEY_REFRESH_TOKEN, options.refreshToken);
  }
